import { map, tileLayer, Icon, icon, marker, popup, latLng, layerGroup } from 'leaflet';
import markerIcon from 'leaflet/dist/images/marker-icon.png';
import markerIcon2x from 'leaflet/dist/images/marker-icon-2x.png';
import markerShadow from 'leaflet/dist/images/marker-shadow.png';
import { MAP_SERVICE_API_KEY, VECTOR_TILE_URL } from '../config';
...
export default class Map {
  ...
  constructor(selector, options = {}) {
    ...
    this.#map = map(document.querySelector(selector), {
      ...
      layers: [tileOsm], // default aktif OpenStreetMap
    });

    const baseMaps = {
      'OpenStreetMap': tileOsm,
      'MapTiler Streets': tileMapTiler,
    };

    L.control.layers(baseMaps).addTo(this.#map); // ini layer control
  }
